...
async function updateCombat(combat, updates, context) {
    ...
    if (previousToken) {
        await new Events.CombatEvent(combat, 'turnEnd', previousToken, {context, combatant: previousCombatant, round: previousRound, turn: previousTurn}).run();
    }
    if (currentToken) {
        for (let token of currentToken.parent.tokens.filter(i => i.actor && ['npc', 'character'].includes(i.actor.type))) {
            await new Events.CombatEvent(combat, 'everyTurn', token, {context, combatant: currentCombatant, round: currentRound, turn: currentTurn, previousCombatant, previousRound, previousTurn}).run();
        }
        await new Events.CombatEvent(combat, 'turnStart', currentToken, {context, combatant: currentCombatant, round: currentRound, turn: currentTurn, previousCombatant, previousRound, previousTurn}).run();
    }
}
async function combatStart(combat, updates) {
    ...
    for (const combatant of combat.combatants) {
        await new Events.CombatEvent(combat, 'combatStart', combatant.token, {combatant, round: currentRound, turn: currentTurn}).run();
    }
}
async function deleteCombat(combat, updates, context) {
    ...
    const previousTurn = combat.previous.turn ?? -1;
    for (const combatant of combat.combatants) {
        await new Events.CombatEvent(combat, 'combatEnd', combatant.token, {context, combatant, round: currentRound, turn: currentTurn, previousCombatant, previousRound, previousTurn}).run();
    }
}
